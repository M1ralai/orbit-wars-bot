...
BASE_PARAMS = {
    ...
    "panic_reserve_mult": 0.8678,
    "neutral_bonus": 19,
    "neutral_tax": 9,
    "enemy_bonus": 26,
    "enemy_weak_bonus": 1.585,
    "counter_bonus": 39,
    "pressure_max": 25,
    "pressure_divisor": 40,
    "production_weight": 43,
    "high_production_weight": 54,
    "high_prod_tax": 26,
    "distance_weight": 2.398,
    "high_distance_weight": 1.1269,
    "short_hop_bonus": 1.2123,
    "short_hop_range": 17,
    "ship_weight": 1.3457,
    "high_ship_weight": 0.9482,
    "overkill": 3,
    "high_prod_extra": 0,
    "enemy_extra": 0,
    "attack_fraction": 0.99,
    "max_attacks_per_turn": 5,
    "comet_bonus": 16,
    "staging_penalty": 10.2345,
    "defense_worth_factor": 8.4622,
    "counter_attack_bonus": 25.6291,
    "production_forecast_mult": 0.7802,
    "evac_eta_threshold": 4.0071,
    "evac_minor_prod": 4,
    "sync_max_eta": 8.7991,
    "sync_min_target_prod": 1,
    "sync_min_target_ships": 40,
    "snipe_max_step": 84,
    "snipe_overkill": 1,
    "honeypot_min_prod": 4,
    "honeypot_reserve": 6,
...
def run_snipe_hijack(
    ...
    for target in attack_targets:
        ...
        for src, min_send, max_send in valid_snipers:
            ...
            src["ships"] -= send
"""
Coordinated multi-source attacks for targets no single planet can take alone,
and finishing waves when far ahead.
"""


def _available_ships(src, net_threats, doomed_planets, P):
    """How many ships a planet can contribute to a sync attack."""
    if src["id"] in doomed_planets:
        return src["ships"]
    threat = net_threats.get(src["id"], 0)
    reserve = reserve_for(src, threat, P)
...
def _eta_from(src, target, send, step, orbit_table):
    """Estimated arrival step from src to target."""
    tx, ty = target["x"], target["y"]
    if target["id"] in orbit_table:
        d0 = dist_xy(src["x"], src["y"], tx, ty)
        tt = travel_time(d0, send)
...
def run_sync_attacks(
    ...
    """
    Phase 1: Coordinated capture of high-value targets that need multiple sources.
    Phase 2: Finishing wave when dominating.
    """
    sync_moves = []

    # --- Phase 1: Multi-source coordinated attacks on big targets ---
    enemy_targets = [t for t in attack_targets if t["owner"] not in (-1, player)]
    # Prioritize by production then weakness
    enemy_targets.sort(key=lambda t: (-t["production"], t["ships"]))

    for target in enemy_targets:
        if target["production"] < P.get("sync_min_target_prod", 3.0):
            continue

        # Gather candidate attackers
        candidates = []
        for src in my_planets:
            avail = _available_ships(src, net_threats, doomed_planets, P)
            if avail <= 0:
                continue
            if path_hits_sun(src["x"], src["y"], target["x"], target["y"]):
                continue
            tt, tx, ty = _eta_from(src, target, avail, step, orbit_table)
            if tt > P.get("sync_max_eta", 15.0):
                continue
            candidates.append((src, avail, tt, tx, ty))

        if len(candidates) < 2:
            continue

        # How many ships needed to take this target?
        # Use the slowest candidate's travel time for production forecast
        candidates.sort(key=lambda c: c[2])  # sort by eta
        max_tt = candidates[-1][2]
        needed = capture_cost(target, player, assigned, P, travel_t=max_tt)
        if needed <= 0:
            continue
        ...
        # Check that no single source can do it alone (otherwise normal pressure handles it)
        best_single = max(c[1] for c in candidates)
        single_tt = next(c[2] for c in candidates if c[1] == best_single)
        single_needed = capture_cost(target, player, assigned, P, travel_t=single_tt)
        if best_single >= single_needed:
            continue  # Normal pressure can handle this

        # Allocate ships from each candidate
        remaining = needed
        attack_plan = []
        for src, avail, tt, tx, ty in candidates:
            if remaining <= 0:
                break
            send = min(avail, remaining)
            if send <= 0:
                continue
            attack_plan.append((src, send, tx, ty))
            remaining -= send

        if remaining > 0:
            continue  # Can't fulfill

        # Execute the coordinated attack
        for src, send, tx, ty in attack_plan:
            angle = aim_angle(src, target, send, step, orbit_table)
            sync_moves.append([src["id"], angle, send])
            assigned[target["id"]] = assigned.get(target["id"], 0) + send
            src["ships"] -= send

    # --- Phase 2: Finishing wave when far ahead ---
    if my_prod >= enemy_prod * 2.0 and enemy_prod > 0:
        # We're dominating - attack everything in reach
        remaining_enemies = [
            t for t in attack_targets
            if t["owner"] not in (-1, player) and t["id"] not in assigned
        ]
        remaining_enemies.sort(key=lambda t: t["ships"])

        for target in remaining_enemies:
            candidates = []
            for src in my_planets:
                avail = _available_ships(src, net_threats, doomed_planets, P)
                if avail <= 2:
            ...
            max_tt = max(c[2] for c in candidates)
            needed = capture_cost(target, player, assigned, P, travel_t=max_tt)
            total_available = sum(c[1] for c in candidates)
            if total_available < needed:
                continue
...
def run_pressure_plan(
    ...
    for src in my_planets:
        ...
        # Reinforce threatened allies — but only if close and worth it
        if not is_doomed:
            for home in threatened_homes[:2]:
                if home["id"] == src["id"] or path_hits_sun(src["x"], src["y"], home["x"], home["y"]):
                    continue
                d = dist_xy(src["x"], src["y"], home["x"], home["y"])
                if d > P["short_hop_range"] * 1.5:
                    continue
                home_deficit = net_threats.get(home["id"], 0)
                if home_deficit <= 0:
                    continue
                send = min(available // 3, home_deficit + 2)
                if send <= 0:
                    continue
                moves.append([src["id"], aim_angle(src, home, send, step, orbit_table), send])
                available -= send
                net_threats[home["id"]] = max(0, home_deficit - send)
                break
        ...
        for score, target, bridge_id in scored[:3]:
            ...
            # Don't send if we can't actually capture
            remaining_def = target["ships"] + int(tt * target["production"]) - assigned.get(target["id"], 0)
            send = min(available, needed)
            if is_doomed:
                send = available
            elif send <= remaining_def * 0.5:
                continue
            if send <= 0:
                continue
