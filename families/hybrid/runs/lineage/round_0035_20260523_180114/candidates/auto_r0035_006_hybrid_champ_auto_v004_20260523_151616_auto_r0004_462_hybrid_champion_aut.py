...
BASE_PARAMS = {
    ...
    "distance_weight": 2.3045,
    "high_distance_weight": 1.4468,
    "ship_weight": 1.4979,
    "high_ship_weight": 0.6058,
    "overkill": 5,
    "high_prod_extra": 0,
    "enemy_extra": 6,
    "comet_bonus": 13,
    "production_forecast_mult": 0.7968,
}
...
# ---------------------------------------------------------------------------
# Doomed detection + evacuation
# ---------------------------------------------------------------------------
def _find_doomed(my_planets, threats_det, reinf_det):
    doomed = set()
    for p in my_planets:
        pid = p["id"]
        ...
        incoming_enemy = sum(s for s, e in t_list if e <= earliest_eta + 1.5)
...
# Guard: enemy fleet heading to neutral, we can arrive 1 step after capture
# ---------------------------------------------------------------------------
def _run_snipe(neutrals, enemy_fleets, my_planets, doomed, threats,
               step, P, orbit_table, moves, assigned):
    if step > 120:  # snipe is an early/mid game tactic
        return
    eta_cache = {}
    for target in neutrals:
        ...
        # Simulate: when does enemy capture? How many ships survive?
        ...
        for src in my_planets:
            ...
            # Must arrive close to our_eta (within 1 step tolerance)
            ...
            break  # one sniper per target


# ---------------------------------------------------------------------------
# Reinforcement: save high-value planets about to fall
# Guard: planet prod >= 3, threat within 5 steps, neighbor close enough
# ---------------------------------------------------------------------------
def _reinforce_valuable(my_planets, threats_det, doomed, threats,
    ...
    for p in my_planets:
        ...
        if earliest_eta > 5:
            continue
        incoming_enemy = sum(s for s, e in t_list if e <= earliest_eta + 2)
        deficit = incoming_enemy - p["ships"] - int(earliest_eta * p["production"])
        if deficit <= 0:
            continue  # can handle it alone

        # Find close neighbors to reinforce
        needed = deficit + int(P["overkill"])
        for nb in my_planets:
            if nb["id"] == pid or nb["id"] in doomed:
                continue
            d = dist_xy(nb["x"], nb["y"], p["x"], p["y"])
            if d > 35:  # only close neighbors
                continue
            if path_hits_sun(nb["x"], nb["y"], p["x"], p["y"]):
                continue
            nb_threat = threats.get(nb["id"], 0)
            nb_reserve = max(P["min_reserve"], int(nb["production"] * P["reserve_prod_mult"])) + nb_threat
            nb_avail = int(nb["ships"] - nb_reserve)
            if nb_avail <= 0:
                continue
            # Can reinforcement arrive in time?
...
# ---------------------------------------------------------------------------
# Sync: coordinated attack on target no single planet can take
# Guard: best single source < needed, total from nearby >= needed
# ---------------------------------------------------------------------------
def _sync_attack(my_planets, enemy_targets, doomed, threats,
                 player, step, P, orbit_table, moves, assigned):
    for target in enemy_targets:
        if target["id"] in assigned:  # already being attacked
            continue
        if target["production"] < 2:  # not worth coordinating for
            continue
        ...
        # Check: can anyone solo it? If yes, normal loop handles it.
        ...
        # Execute: send from each, closest first
...
# ---------------------------------------------------------------------------
# Honeypot: when ahead in late game, let enemy attack then crush them
# Guard: step >= MID_END, my_prod > enemy_prod * 1.3, planet under attack,
#        neighbors can reinforce before enemy arrives
# ---------------------------------------------------------------------------
def _honeypot_reinforce(my_planets, threats_det, doomed, threats,
                        my_prod, enemy_prod, step, P, orbit_table, moves):
    if step < MID_END or my_prod <= enemy_prod * 1.3:
        return
    ...
    for nb in my_planets:
        ...
        send = min(nb_avail, needed)
        tt = travel_time(d, send)
        if tt > earliest_eta:  # must arrive before enemy
            continue
...
def agent(obs):
    ...
    # Orbit-aware threat detection
    threats = detect_threats(my_planets, enemy_fleets, step, orbit_table)
    eta_cache = {}
    ...
    doomed = _find_doomed(my_planets, threats_det, reinf_det)
    ...
    # --- Tactic 1: Snipe (early/mid, enemy fleet -> neutral) ---
    _run_snipe(neutrals, enemy_fleets, my_planets, doomed, threats,
               step, P, orbit_table, moves, assigned)

    # --- Tactic 2: Reinforce valuable planets about to fall ---
    _reinforce_valuable(my_planets, threats_det, doomed, threats,
                        step, P, orbit_table, moves)

    # --- Tactic 3: Honeypot (late game, ahead, planet under attack) ---
    _honeypot_reinforce(my_planets, threats_det, doomed, threats,
                        my_prod, enemy_prod, step, P, orbit_table, moves)

    # --- Core attack loop (same as simple bot) ---
    ...
    # --- Tactic 4: Sync attack (targets nobody could solo) ---
    _sync_attack(my_planets, enemy_targets, doomed, threats,
                 player, step, P, orbit_table, moves, assigned)
