...
BASE_PARAMS = {
    "min_ships": 20,
    "min_reserve": 2,
    "reserve_prod_mult": 1.6901,
    "panic_reserve_mult": 0.1988,
    "neutral_bonus": 31,
    "neutral_tax": 2,
    "enemy_bonus": 28,
    "enemy_weak_bonus": 0.357,
    "counter_bonus": 2,
    "pressure_max": 2,
    "pressure_divisor": 51,
    ...
    "high_prod_tax": 1,
    "distance_weight": 2.1156,
    "high_distance_weight": 2.0259,
    "short_hop_bonus": 0.3892,
    "short_hop_range": 18,
    "ship_weight": 1.1779,
    "high_ship_weight": 0.7233,
    "overkill": 2,
    "high_prod_extra": 4,
    "enemy_extra": 14,
    "attack_fraction": 0.9362,
    "max_attacks_per_turn": 7,
    "comet_bonus": 7,
    "staging_penalty": 20.0,
    "defense_worth_factor": 12.3879,
    "counter_attack_bonus": 26.7463,
}
...
def agent(obs):
    ...
    honeypot_id = None
    if late_game_ahead:
        candidates = [p for p in my_planets if p["production"] >= 3]
        if not candidates:
            candidates = [p for p in my_planets if p["production"] >= 2]
        if candidates:
    ...
    if early_game:
        ...
        for target in attack_targets:
            ...
            if enemy_ships > neutral_ships:
                ...
                S_needed = enemy_total + P.get("overkill", 3)
    ...
    if late_game_ahead and honeypot_id is not None and threats_det.get(honeypot_id, []):
        ...
        if hp_planet:
            ...
            if deficit > 0:
                needed_reinf = deficit + P.get("overkill", 3)
    ...
    if late_game_behind and (step % 6 == 0):
