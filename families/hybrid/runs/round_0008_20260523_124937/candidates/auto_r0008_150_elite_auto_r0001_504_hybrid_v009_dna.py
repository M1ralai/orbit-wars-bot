...
BASE_PARAMS = {
    "min_ships": 17,
    "min_reserve": 2,
    "reserve_prod_mult": 1.6343,
    "panic_reserve_mult": 0.0,
    "neutral_bonus": 31,
    "neutral_tax": 0,
    "enemy_bonus": 20,
    "enemy_weak_bonus": 0.2161,
    "counter_bonus": 0,
    "pressure_max": 4,
    "pressure_divisor": 14,
    "production_weight": 41,
    "high_production_weight": 57,
    "high_prod_tax": 3,
    "distance_weight": 2.0269,
    "high_distance_weight": 1.8825,
    "short_hop_bonus": 0.2308,
    "short_hop_range": 16,
    "ship_weight": 1.9367,
    "high_ship_weight": 0.9263,
    "overkill": 7,
    "high_prod_extra": 1,
    "enemy_extra": 3,
    "attack_fraction": 0.9726,
    "max_attacks_per_turn": 9,
    "comet_bonus": 14,
    "staging_penalty": 19.7939,
    "defense_worth_factor": 12.0,
    "counter_attack_bonus": 17.5246,
    "production_forecast_mult": 1.0222,
    "evac_eta_threshold": 3.7203,
    "evac_minor_prod": 2,
    "sync_max_eta": 13.2149,
...
def score_target(src, target, player, step, assigned, P, comet_ids, orbit_table, threats, available, my_planets, counter_attack_targets):
    ...
    # Prevent scoring and attempting underpowered attacks that cannot conquer the target's remaining defense
...
def agent(obs):
    ...
    if early_game:
        ...
        for target in attack_targets:
            ...
            if enemy_ships > neutral_ships:
                ...
                # Choose the single sniper that can satisfy the full cost on its own to prevent speed-mismatch bugs
                best_sniper = None
                for src, avail in valid_snipers:
                    if avail >= S_needed:
                        if best_sniper is None or avail > best_sniper[1]:
                            best_sniper = (src, avail)
                
                if best_sniper:
                    src, avail = best_sniper
                    send = S_needed
    ...
    if late_game_ahead and honeypot_id is not None and threats_det.get(honeypot_id, []):
        ...
        if hp_planet:
            ...
            if deficit > 0:
                ...
                # Find a single neighbor that can cover the entire reinforcement on its own
                best_neighbor = None
                for neighbor in my_planets:
                    ...
                    if step + eta == K:
                        if best_neighbor is None or neigh_avail > best_neighbor[1]:
                            best_neighbor = (neighbor, neigh_avail, pred_hp)
                
                if best_neighbor:
                    neighbor, avail, pred_hp = best_neighbor
                    send = needed_reinf
    ...
    for src in my_planets:
        ...
        # Doomed Planet Fallback Evacuation (if no threatened homes nearby, save ships to our safest friendly planet)
        if is_doomed and available > 0:
            safe_planets = [p for p in my_planets if p["id"] != src["id"] and p["id"] not in doomed_planets]
            if safe_planets:
                # Sort by net threat (ascending), then by production (descending), then by ships (descending)
                safe_planets.sort(key=lambda p: (net_threats.get(p["id"], 0), -p["production"], -p["ships"]))
                target_safe = safe_planets[0]
                if not path_hits_sun(src["x"], src["y"], target_safe["x"], target_safe["y"]):
                    send = available
                    moves.append([src["id"], aim_angle(src, target_safe, send, step, orbit_table), send])
                    available = 0
        ...
        for score, target, bridge_id in scored[:3]:
            ...
            # Prevent launching underpowered attacks that cannot capture the target's remaining defense
            if not is_doomed:
                remaining_def = target["ships"] - assigned.get(target["id"], 0)
                if send <= remaining_def:
                    continue
