...
BASE_PARAMS = {
    "min_ships": 17,
    "min_reserve": 2,
    "reserve_prod_mult": 1.9826,
    "panic_reserve_mult": 0.0535,
    "neutral_bonus": 31,
    "neutral_tax": 1,
    "enemy_bonus": 24,
    ...
    "high_prod_tax": 5,
    "distance_weight": 2.151,
    "high_distance_weight": 2.151,
    "short_hop_bonus": 0.0,
    "short_hop_range": 16,
    "ship_weight": 1.1722,
    "high_ship_weight": 0.6815,
    "overkill": 3,
    "high_prod_extra": 5,
    "enemy_extra": 11,
    ...
    "comet_bonus": 7,
    "staging_penalty": 14.6718,
    "defense_worth_factor": 8.8482,
    "counter_attack_bonus": 18.1116,
    "production_forecast_mult": 0.8042,
    "evac_eta_threshold": 3.8086,
    "evac_minor_prod": 3,
    "sync_max_eta": 13.1154,
    "sync_min_target_prod": 5,
...
def fleet_hit_eta(fleet, planet, step=None, orbit_table=None, max_steps=120):
    ...
    if step is not None and orbit_table is not None:
        for eta in range(1, max_steps + 1):
            fleet_x = fx + eta * speed * fdx
            fleet_y = fy + eta * speed * fdy
            if planet["id"] in orbit_table:
                px, py = predict_pos(planet["id"], step + eta, orbit_table)
            else:
                px, py = planet["x"], planet["y"]
            if dist_xy(fleet_x, fleet_y, px, py) < planet["radius"] + 2.5:
                return eta
        return None
    ...
    if perp < planet["radius"] + 2.5:
        return dist_xy(fx, fy, px, py) / speed
    return None


def detect_threats(my_planets, enemy_fleets, step=None, orbit_table=None):
    threats = {}
    for fleet in enemy_fleets:
        for planet in my_planets:
            if fleet_hit_eta(fleet, planet, step, orbit_table) is not None:
                threats[planet["id"]] = threats.get(planet["id"], 0) + fleet["ships"]
    return threats


def detect_reinforcements(my_planets, my_fleets, step=None, orbit_table=None):
    reinforcements = {}
...
def detect_threats_detailed(my_planets, enemy_fleets, step=None, orbit_table=None):
    details = {}
    for fleet in enemy_fleets:
        for planet in my_planets:
            eta = fleet_hit_eta(fleet, planet, step, orbit_table)
            if eta is not None:
...
def detect_reinforcements_detailed(my_planets, my_fleets, step=None, orbit_table=None):
    details = {}
    for fleet in my_fleets:
        for planet in my_planets:
            eta = fleet_hit_eta(fleet, planet, step, orbit_table)
...
def fleet_hits_planet_eta(fleet, planet, step, orbit_table, max_steps=120):
    speed = fleet_speed(fleet["ships"])
    if speed <= 0:
        return None
    ...
    for eta in range(1, max_steps + 1):
        fleet_x = fx + eta * speed * dx
        fleet_y = fy + eta * speed * dy
        if planet["id"] in orbit_table:
            px, py = predict_pos(planet["id"], step + eta, orbit_table)
        else:
            px, py = planet["x"], planet["y"]

        if dist_xy(fleet_x, fleet_y, px, py) < hit_radius:
            return eta

    return None


def detect_counter_attack_targets(enemy_fleets, my_planets, step, orbit_table):
    counter_attack_targets = set()
    for fleet in enemy_fleets:
        if fleet["from_planet"] < 0:
            continue
        for planet in my_planets:
            if fleet_hits_planet_eta(fleet, planet, step, orbit_table) is not None:
...
def run_feint(attack_targets, my_planets, my_fleets, late_game_behind, step, P, orbit_table, doomed_planets, moves):
    ...
    for target in enemy_fortresses:
        ...
        for fleet in my_fleets:
            if fleet["ships"] < 1:
                continue
            if fleet_hits_planet_eta(fleet, target, step, orbit_table) is not None:
    ...
    for src in my_planets:
        ...
        for score, target, bridge_id in scored[:3]:
            ...
            for fleet in my_fleets:
                if fleet["ships"] < 4:
                    continue
                eta = fleet_hits_planet_eta(fleet, target, step, orbit_table)
                if eta is not None:
...
def agent(obs):
    ...
    reinforcements = detect_reinforcements(my_planets, my_fleets, step, orbit_table)
    threats_det = detect_threats_detailed(my_planets, enemy_fleets, step, orbit_table)
    reinforcements_det = detect_reinforcements_detailed(my_planets, my_fleets, step, orbit_table)
    ...
    counter_attack_targets = detect_counter_attack_targets(enemy_fleets, my_planets, step, orbit_table)
