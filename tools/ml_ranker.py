...
def base_family(base):
    if not base:
        return "unknown"
    if base == "champion_template" or base.endswith("_champion_template"):
...
def collect_training_rows(output_dir=Path("auto_runs"), min_games=20, max_rounds=None):
...
def build_training_arrays(rows, priors=None):
    ...
    sample_weight = np.array(
        [max(0.35, min(2.0, float(row.get("games", 0)) / 60.0)) for row in rows],
        dtype=float,
    )
...
def parse_args():
    ...
    dataset.add_argument("--output-dir", type=Path, default=Path("auto_runs"))
    dataset.add_argument("--dataset-path", type=Path, default=Path("training/ml_dataset.jsonl"))
    dataset.add_argument("--min-games", type=int, default=20)
    dataset.add_argument("--max-rounds", type=int, default=None)

    train = subparsers.add_parser("train")
    train.add_argument("--output-dir", type=Path, default=Path("auto_runs"))
    ...
    train.add_argument("--min-games", type=int, default=20)
